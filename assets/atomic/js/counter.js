...
const isEditMode = () => !!(window.elementorFrontend?.isEditMode?.() || document.body?.classList.contains('elementor-editor-active'));
...
const play = (parent, numberEl) => {
  ...
  const duration = Number.isFinite(durationAttr) && durationAttr > 0 ? durationAttr : DEFAULT_DURATION;
  const write = value => {
