...
const isIconEl = el => el.classList.contains('e-svg-base') || !!el.querySelector(':scope > svg');
...
const ensureHooks = item => {
  ...
  const headerDiv = open && open.parentElement || icons[0] && icons[0].parentElement || headerSlot && headerSlot.firstElementChild;
...
const ensureButtonLabel = item => {
  ...
  const named = btn.textContent.trim().length > 0 || btn.hasAttribute('aria-labelledby');
  if (named || !fallback) {
...
const isEditor = doc => !!(doc && doc.body && doc.body.classList.contains('elementor-editor-active'));
...
const bootstrap = doc => {
  ...
  const isAccordionish = node => node.nodeType === 1 && (node.classList.contains('aae-a-accordion') || node.classList.contains('aae-a-accordion-item') || !!node.querySelector('.aae-a-accordion'));
  let scheduled = false;
  ...
  docObserver.observe(doc.body || doc.documentElement, {
    childList: true,
    subtree: true
  });
};
