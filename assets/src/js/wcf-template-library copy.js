...
(function ($, window, document, config) { 
    ...
    let aae_domain = 'https://crowdytheme.com/elementor/info-templates/wp-json/wp/v2/wcf-templates?page=1&per_page=20&subtype=block'
    ...
    const get_category_templates = async function (category = '', type, load=0) {   
     ...
     const search_category_templates = async function (text = '') {
        let type          = $('#elementor-template-library-header-menu .elementor-active').attr('data-tab') || 'block';   
        let result = [];
        let query_domain = new URL(aae_domain);
        if (type) { 
            query_domain.searchParams.set('subtype', type);
        ...
        elementor.on("preview:loaded", function () {
            $(elementor.$previewContents[0].body).on("click", ".elementor-add-wcf-template-button", function (event) {
              ...
              async function render_templates(t, activeMenu, category='') {
                    ...
                    const currentchunk = await get_category_templates(category, activeMenu,0);
                ...
                function render_single_template(t) {
                  ...
                  const backContent = $('#wcf-template-library .dialog-widget-content').html();
                    $(document).on('click', '.thumbnail', function () {
                        ...
                        let is_loading = true;
                        loading(is_loading);                       
                        $('.wcf-library-templates').html(`<iframe src="${cleanedUrl}"></iframe>`);
