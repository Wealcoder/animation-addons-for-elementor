...
document.addEventListener('DOMContentLoaded', () = > {
		...
		const collectAllImagesInIframe = (doc) = > {
			...
			return {
				imgs, urls: uniq(urls).filter((u) = > !u.startsWith("data:"))
			};
		};
					...
					try {
						if ("decode" in img) {
							...
							await new Promise((res) = > {
								img.complete ? res() : img.addEventListener("load", res, {
									once: true
								});
							});
						}
			...
			const extractCrowdyUrls = (items) = > {
				...
				self.onmessage = async(e) = > {
					...
					try {
						...
						postMessage({
							urls: uniq(allUrls).filter(u = > !/^data:/i.test(u))
						});
					} catch (err) {
						postMessage({
							urls: []
