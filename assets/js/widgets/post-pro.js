(function($) {
		const WCF_Feature_Posts = function($scope, $) {
			...
			const open_popup = $(`.elementor - element - $ {
				$popup_id
			}.wcf - post - popup, .wcf - post - popup`);
			...
			open_popup.each(function(index) {
					...
					$(this).off('click').on('click', function() {
							...
							if ($(this).hasClass('audio')) {
								$('.wcf--popup-video-wrapper').find('.aae-popup-content-container').html(` < div class = "audio wcf-audio-wrapper-clean" > < audio controls > < source src = "${$_url}"
        ...
        / / Calculate audio duration $('.audio-duration-wrapper', $scope).each(function() {
										const wrapper = $(this);
										...
										if (link) {
											...
											audio.addEventListener('loadedmetadata', function() {
												...
												wrapper.find('.audio-duration').text(`$ {
														minutes
													}: $ {
														seconds < 10 ? '0' : ''
													}
											...
											audio.addEventListener('error', function() {
												console.error(`Could not load audio from: $ {
													link
												}`);
								...
								$(window).on('elementor/frontend/init', function() {
									...
									elementorFrontend.hooks.addAction(`frontend / element_ready / wcf--filterable - gallery.
										default`, WCF_Feature_Posts);
									elementorFrontend.hooks.addAction(`frontend / element_ready / wcf--posts - filter.
										default`, WCF_Feature_Posts);
									elementorFrontend.hooks.addAction(`frontend / element_ready / wcf--posts - slider.
										default`, WCF_Feature_Posts);
									elementorFrontend.hooks.addAction(`frontend / element_ready / wcf--posts - timeline.
										default`, WCF_Feature_Posts);
								});
