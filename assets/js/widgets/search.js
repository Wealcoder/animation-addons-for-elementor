(function($) {
		...
		const WcfAjaxSearch = function($scope) {
			...
			$categoryListItems.on('click', function() {
					...
					selectedCategories.forEach(cat = > {
						$categoryContainer.append(` < input type = "hidden"
							name = "category[]"
							value = "${cat.value}" / > `);
					});
					...
					if (selectedCategories.length) {
						$display.html(
							selectedCategories.map(c = > ` < span class = "category-pill" > $ {
								c.label
							} < /span>`).join(', ')
                );
            } else {
        ...
        / / === = Close dropdowns
						...
						// Ajax Search
						const $inputField = $scope.find('.search-field'); const $resultBox = $scope.find('.aae--live-search-results'); const $searchWrapper = $('.search--wrapper.style-full-screen .wcf-search-container');
