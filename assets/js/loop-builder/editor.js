...
(function($) {
		...
		class AAE_LoopBuilderEditor {
			...
			addTemplateManagementUI(model) {
				...
				if ($templateControl.length && !$templateControl.find('.aae-template-actions').length) {
					const $actions = $(` < div class = "aae-template-actions"
						style = "margin-top: 10px;" > < button type = "button"
						class = "elementor-button elementor-button-default aae-create-template" > < i class = "eicon-plus" > < /i> Create Template
                        </button > < button type = "button"
						class = "elementor-button elementor-button-default aae-edit-template"
						style = "display: none;" > < i class = "eicon-edit" > < /i> Edit Template
                        </button > < /div>
                `);
        ...
        createNewTemplate(model) {
            / / Show Elementor - style modal
						for template creation const modal = elementorCommon.dialogsManager.createWidget('confirm', {
						...
						if (data.template_data && Array.isArray(data.template_data)) {
							...
							data.template_data.forEach(element = > {
								if (element.widgetType) {
									...
									previewHtml += ` < div class = "widget-preview" > < i class = "${widgetIcon}" > < /i> ${this.getWidgetLabel(element.widgetType)}
                        </div > `;
								}
							});
							previewHtml += '</div>';
