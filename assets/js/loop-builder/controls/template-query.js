	...
	class AAE_TemplateQueryControl extends elementor.modules.controls.Select2 {
		...
		doCreateTemplate() {
			...
			return new Promise((resolve, reject) = > {
				...
				$.ajax({
					...
					const errorMessage = error.responseJSON && error.responseJSON.data ? error.responseJSON.data : 'Failed to create template. Please try again.';
					elementor.notifications.showToast({
						message: errorMessage,
						type: 'error'
					});
		...
		openTemplateEditorWithReload(templateId) {
			...
			/wp-admin/post.php ? post = $ {
				currentPostId
			} & action = elementor & active - document = $ {
				templateId
			}`;
