...
function showNotification(message) {
    ...
    setTimeout(() = > notification.style.opacity = '1',
	10);
...
function initializePHPVersionCheck() {
	...
	if (typeof WCFCustomCodeVars === 'undefined' || !WCFCustomCodeVars.serverDetails || !WCFCustomCodeVars.serverDetails.currentVersion) {
	...
	function checkPHPVersion() {
		if (codeTypeSelect && codeTypeSelect.value === 'php') {
			...
			if (notice) {
				notice.innerHTML = `Server is running < strong > PHP $ {
					phpVersion
				} < /strong>. Please ensure your code is compatible.`;
            }
        }
    }
    ...
    const featureChecks = [
        ...
        // === PHP 7.2 ===
        { regex: /object\s+\$[A-Za-z_]/, min: 7.2,
				name: 'object type hint'
			}, {
				regex: /(?<!::)count\(/,
				min: 7.2,
				name: 'count() with Countable objects'
			}, {
				regex: /stream_isatty\s*\(/,
				min: 7.2,
				name: 'stream_isatty() function'
			},

			// === PHP 7.3 ===
			{
				regex: /array_key_first\s*\(/,
				min: 7.3,
				name: 'array_key_first()'
			}, {
				regex: /array_key_last\s*\(/,
				min: 7.3,
				name: 'array_key_last()'
			}, {
				regex: /hrtime\s*\(/,
				min: 7.3,
				name: 'hrtime() function'
			},
			...
			// === PHP 7.4 ===
			{
				...
				min: 7.4,
				name: 'Nullable property types'
			}, {
				regex: /[A-Za-z0-9_]+\s+:\s*[A-Za-z0-9_]+(\s*\|[A-Za-z0-9_]+)+/,
				min: 8.0,
				name: 'Union types (PHP 8.0, but check early)'
			}, {
				regex: /Typed property/,
				min: 7.4,
			...
			// === PHP 8.0 ===
			{
				...
				regex: /#[A-Za-z0-9_]+/,
				min: 8.0,
				name: 'Attributes syntax (#[])"'
			}, {
				regex: /str_contains\s*\(/,
				min: 8.0,
				name: 'str_contains()'
			}, {
				regex: /str_starts_with\s*\(/,
				min: 8.0,
				name: 'str_starts_with()'
			}, {
				regex: /str_ends_with\s*\(/,
				min: 8.0,
				name: 'str_ends_with()'
			}, {
				regex: /get_debug_type\s*\(/,
				min: 8.0,
				name: 'get_debug_type()'
			}, {
				regex: /fdiv\s*\(/,
			...
			// === PHP 8.1 ===
			{
				...
				regex: /never\s+function/,
				min: 8.1,
				name: 'never return type'
			}, {
				regex: /array_is_list\s*\(/,
				min: 8.1,
				name: 'array_is_list()'
			},
			...
			// === PHP 8.2 ===
			{
				regex: /readonly\s+class/,
				min: 8.2,
				name: 'Readonly classes'
			}, {
				regex: /true\s*\|\s*false/,
				min: 8.2,
				name: 'true/false as standalone types'
			}, {
				regex: /null\s*\|/,
				min: 8.2,
				name: 'null as standalone type'
			}, {
				regex: /#[A-Za-z0-9_]+\(.+\)/,
				min: 8.2,
				name: 'Disjunctive Normal Form types (DNF)'
			},

			// === PHP 8.3 ===
			{
				regex: /json_validate\s*\(/,
				min: 8.3,
				name: 'json_validate()'
			}, {
				regex: /#[A-Za-z0-9_]+\(.+\)/,
				min: 8.3,
				name: 'Explicitly negative keys in array destructuring'
			},
		...
		function preventHighVersionUsage() {
			...
			featureChecks.forEach(feature = > {
				if (currentVersion < feature.min && feature.regex.test(code)) {
					alert(`Your code uses $ {
						feature.name
					}, which requires PHP $ {
						feature.min
					} + but the server is running PHP $ {
						phpVersion
					}.`);
					blocked = true;
				}
	...
	function initializeVisibilityOptions() {
		...
		function updateVisibilityOptions() {
			...
			if (selectedCodeType === 'php') {
				...
				visibilityPageSelect.innerHTML = ` < optgroup label = "Basic" > < option value = "" > None < /option>
                    <option value="global">Entire Website</option > < option value = "frontend" > Frontend < /option>
                    <option value="admin">Backend</option > < /optgroup>
            `;
        } else {
