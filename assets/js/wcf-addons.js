...
(function($) {
				...
				const Countdown = Modules.extend({
						...
						timer_content: function timer_content(times = []) {
							...
							$.each(times, (index, time) = > {
									const title = this.getElementSettings(`countdown_timer_$ {
											index
										}
										_label`
									);
									time_content += ` < div class = "timer-content timer-item-${index} " > < span class = "time-count ${index}-count" > $ {
										time
									} < /span><span class="time-title ${index}-title">${title}</span > < /div>`;
        });
      ...
      time_finish_content: function () {
        ...
        if (title) {
          finish_content += `<div class="countdown-expire-title">${title}</div > `;
								...
								if (description) {
									finish_content += ` < div class = "countdown-expire-desc" > $ {
										description
									} < /div>`;
        }
        finish_content += "</div > ";
    ...
    const toggle_switcher = function ($scope) {
      ...
      checked.change(function () {
        toggle_pane.toggleClass("
									show ");
        toggle_label.toggleClass("
									active ");
      });
    };
    ...
    const image_accordion = Modules.extend({
      run: function run() {
        let expand = this.getElementSettings("
									expand_style ");
    ...
    elementorFrontend.hooks.addAction(
      "
									frontend / element_ready / wcf--imag - accordion.
									default ",
      function ($scope) {
    ...
    elementorFrontend.hooks.addAction(
      "
									frontend / element_ready / wcf--countdown.
									default ",
      function ($scope) {
