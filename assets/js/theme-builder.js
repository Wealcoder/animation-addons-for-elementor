	...
	const WCFThemeBuilder = {
		...
		savePost: function(event) {
			...
			if (tmpType === "popup") {
				...
				data["tmpEffect"] = $("#wcf-addons--popup--builder-effect").val() ? $("#wcf-addons--popup--builder-effect").val() : "flip";

				data["tmpSelector"] = $("#aae-popup-builder-selector").val() ? $("#aae-popup-builder-selector").val() : "";

				data["tmpScrollPostion"] = $("#aae-popup-builder-scrollPostion").val() ? $("#aae-popup-builder-scrollPostion").val() : 0;
			}

			if (tmpType === 'header') {

				data["tmpHeaderSmoother"] = $("#aae-header-smoother-location").val() ? $("#aae-header-smoother-location").val() : "";

				data["tmpHeaderSmootherOffsetY"] = $("#aae-header-smoother-yoffset").val() ? $("#aae-header-smoother-yoffset").val() : "";
			}
