...
(function($, window, document, config) {
		...
		const get_category_templates = async
		function(
			category = "",
			type,
			page = 1,
		...
		//get specific category templates
		const search_category_templates = async
		function(text = "") {
			let type =
						...
						function template_import(id = null) {
							...
							$(document).on("click", ".library--action.insert", function() {
								...
								if (jurl) {
									...
									$.get({
										url: "https://block.animation-addons.com/wp-json/wp/v2/wcf-templates/json-content?url=" + jurl,
										crossDomain: true
									})
										.done(function(data) {
											if (data ? .content) {
		...
		const generateTemplate = (item) = > {
			return` <div class = "wcf-library-template"
			data - jurl = "${item.json_url}"
			data - id = "${item.id}"
			data - url = "${
      item.template_demo_url
    }" > < div class = "thumbnail" > < img src = "${item?.preview?.url}"
			alt = "${item.title}" > < /div>
                ...
                `
                    ...
                    ${
                      !WCF_TEMPLATE_LIBRARY?.pro_installed
                        ...
                        <a href="https:/ / animation - addons.com " class="
			library--action pro " target="
			_blank ">
                            <i class="
			eicon - external - link - square "></i>
                            Go Premium
                        </a>
                    `
                        : "
			"
                    ...
                    ${
                      ...
                      !WCF_TEMPLATE_LIBRARY?.config?.wcf_valid
                        ...
                        <a href="
			$ {
				WCF_TEMPLATE_LIBRARY.dashboard_link
			}
			" class="
			library--action pro " target="
			_blank ">
			...
			"
                    ...
                    ${
                      ...
                      !WCF_TEMPLATE_LIBRARY?.pro_active
                        ...
                        <button class="
			library--action pro aaeplugin - activate ">
                            <i class="
			eicon - external - link - square "></i>
                            Activate
                        </button>
