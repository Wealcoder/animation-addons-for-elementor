...
final class Kses {
	...
	public static function builder_html( $html ) {
		...
		$styles = array();
		$token  = '@@AAEKSES' . wp_rand( 100000, 999999 ) . 'S';
		...
		// are kept beside kses the way <style> is, with the one sequence that
		// could end the block early neutralised. Every executable script goes,
		// with its contents -- kses alone would leave the source on the page
		// as visible text.
		$html = preg_replace_callback(
			'#<script\b([^>]*)>(.*?)</script\s*>#is',
			static function ( $m ) use ( &$styles, $token ) {
				if ( ! self::is_data_script( $m[1] ) ) {
					return '';
				}
				$i            = count( $styles );
				$styles[ $i ] = '<script' . self::script_attributes( $m[1] ) . '>' . str_replace( '</', '<\/', $m[2] ) . '</script>';
				return $token . $i . '@@';
			},
			$html
		);
		...
		$html = preg_replace_callback(
			...
			static function ( $m ) use ( &$styles, $token ) {
				$i            = count( $styles );
				$styles[ $i ] = '<style' . self::style_attributes( $m[1] ) . '>' . wp_strip_all_tags( $m[2] ) . '</style>';
				return $token . $i . '@@';
			},
			$html
		);
		...
		if ( $styles ) {
			$html = preg_replace_callback(
				...
				static function ( $m ) use ( $styles ) {
					return isset( $styles[ (int) $m[1] ] ) ? $styles[ (int) $m[1] ] : '';
				},
				$html
			);
		}
	 ...
	 * data-* hooks a runtime finds it by. Each escaped; the tag never meets
	 * kses.
	 *
	 * @param string $raw The attribute string after `<script`.
	 * @return string
	...
	/**
	 * Keep only the attributes a `<style>` tag legitimately carries, each
	 * escaped -- the tag itself never goes through kses.
	 *
	...
	private static function style_attributes( $raw ) {
		...
		if ( preg_match_all( '/\b(id|media|type)\s*=\s*("([^"]*)"|\'([^\']*)\')/i', $raw, $m, PREG_SET_ORDER ) ) {
			foreach ( $m as $a ) {
				$value = isset( $a[4] ) && '' !== $a[4] ? $a[4] : $a[3];
				$out  .= ' ' . strtolower( $a[1] ) . '="' . esc_attr( $value ) . '"';
			}
		}
		return $out;
	}
