	...
	function aae_addon_breadcrumbs($tag = 'div', $separator = ' &raquo; ')
	{
		...
		if (is_category() || (is_single() && get_post_type() === 'post')) {
			...
			if ($post_type && $post_type->has_archive) {
				echo '<a href="' . esc_url(get_post_type_archive_link(get_post_type())) . '">' . esc_html($post_type->labels->name) . '</a>' . wp_kses_post($separator);
			}
			...
			foreach ($taxonomies as $taxonomy) {
				...
				if (!empty($terms) && !is_wp_error($terms)) {
					$term = current($terms);
					echo '<a href="' . esc_url(get_term_link($term)) . '">' . esc_html($term->name) . '</a>' . wp_kses_post($separator);
					break;
				}
			}
