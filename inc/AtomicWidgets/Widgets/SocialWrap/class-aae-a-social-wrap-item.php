...
/**
 * AAE Social Wrap Item — a single OPEN link container (icon + label) meant
 * to live inside AAE_A_Social_Wrap, or stand on its own.
 *
 * Unlike AAE_A_Social_Share_Item (locked, vendor-enum driven, styled from
 ...
 */
class AAE_A_Social_Wrap_Item extends Atomic_Element_Base {

	use Has_Element_Template;

	const BASE_STYLE_KEY = 'base';

	public static $widget_description = 'An open, freely editable share-link item (icon + label). Duplicate it inside an AAE Social Wrap to build a custom social-share row, or use it standalone as any icon+label link.';
	...
	public static function get_type() {
		return 'e-aae-a-social-wrap-item';
	}
	...
	public function get_title() {
		return esc_html__( 'AAE Social Wrap Item', 'animation-addons-for-elementor' );
	}
	...
	public static function build_default_inner_children( string $vendor = 'facebook', string $label = 'Share' ): array {
		$svg_settings = [
			'classes' => Classes_Prop_Type::generate( [ 'aae-a-social-wrap-item-icon' ] ),
		];
		...
		$title = Atomic_Paragraph::generate()
			...
			->settings( [
				'classes'   => Classes_Prop_Type::generate( [ 'aae-a-social-wrap-item-label' ] ),
	...
	protected function get_templates(): array {
		return [
			'elementor/elements/aae-a-social-wrap-item' => __DIR__ . '/aae-a-social-wrap-item.html.twig',
		];
	}
	...
	public static function get_vendor_svg_url( string $vendor ): string {
		...
		return WCF_ADDONS_URL . 'inc/AtomicWidgets/Widgets/SocialWrap/assets/svg/' . $vendor . '.svg';
	}
}
