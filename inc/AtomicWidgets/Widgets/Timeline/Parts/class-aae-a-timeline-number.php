...
/**
 * AAE Timeline — Number. A single small, muted leaf label (the "01", "02"…
 * milestone index). Its own widget type exists ONLY so it can carry its own
 * fixed typography via define_base_styles() — reusing the core e-paragraph
...
class AAE_A_Timeline_Number extends Atomic_Widget_Base {
	...
	public static $widget_description = 'Internal milestone-index label used by the AAE Timeline item.';

	public static function get_element_type(): string {
		return 'e-aae-a-timeline-number';
	}

	public function get_title() {
		return esc_html__( 'Timeline Number', 'animation-addons-for-elementor' );
	}

	public function get_keywords() {
		return [ 'atomic', 'timeline', 'number', 'index' ];
	}
	...
	protected static function define_props_schema(): array {
		return [
			...
			'text' => Html_V3_Prop_Type::make()
				->default( [
					'content'  => String_Prop_Type::generate( '01' ),
					'children' => [],
				] )
				->description( 'The milestone index text.' ),
			...
			Section::make()
				...
				->set_items( [
					Inline_Editing_Control::bind_to( 'text' )
						->set_label( __( 'Number', 'animation-addons-for-elementor' ) ),
				] ),
			Section::make()
	...
	protected function get_templates(): array {
		return [
			'elementor/elements/aae-a-timeline-number' => __DIR__ . '/aae-a-timeline-number.html.twig',
		];
	}
}
