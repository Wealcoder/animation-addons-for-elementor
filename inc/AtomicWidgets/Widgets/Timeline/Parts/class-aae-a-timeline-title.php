...
/**
 * AAE Timeline — Title. A single bold milestone heading. Its own widget type
 * exists ONLY so it can carry its own fixed typography via
 * define_base_styles() — see class-aae-a-timeline-number.php for why this
 * can't be a reused e-heading.
 */
class Aaeaddon_A_Timeline_Title extends Atomic_Widget_Base {

	use Has_Template;

	public static $widget_description = 'Internal milestone-title label used by the AAE Timeline item.';

	public static function get_element_type(): string {
		return 'e-aae-a-timeline-title';
	}

	public function get_title() {
		return esc_html__( 'Timeline Title', 'animation-addons-for-elementor' );
	}

	public function get_keywords() {
		return [ 'atomic', 'timeline', 'title', 'heading' ];
	}
	...
	protected static function define_props_schema(): array {
		return [
			...
			'text' => Html_V3_Prop_Type::make()
				->default( [
					'content'  => String_Prop_Type::generate( 'Event Title' ),
					'children' => [],
				] )
				->description( 'The milestone title text.' ),
			...
			'base' => Style_Definition::make()
				->add_variant(
					Style_Variant::make()
						...
						->add_prop( 'font-size',     Size_Prop_Type::generate( [ 'size' => 16, 'unit' => 'px' ] ) )
	...
	protected function get_templates(): array {
		return [
			'elementor/elements/aae-a-timeline-title' => __DIR__ . '/aae-a-timeline-title.html.twig',
		];
	}
}
