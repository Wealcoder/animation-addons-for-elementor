<?php
/**
 * AAE Post Pagination Preview — Author. See class-aae-a-post-pagination-
 * preview-image.php's docblock for the shared `role`/Render_Context pattern,
 * and class-aae-a-post-pagination-preview-date.php for why this is
 * `inline-block` (sits on the same line as Date, its default predecessor).
 *
...
class AAE_A_Post_Pagination_Preview_Author extends Atomic_Widget_Base {
	...
	public static function get_element_type(): string {
		return 'e-aae-a-post-pagination-preview-author';
	}

	public function get_title() {
		return esc_html__( 'Preview Author', 'animation-addons-for-elementor' );
	}
	...
	protected function get_templates(): array {
		return [
			'elementor/elements/aae-a-post-pagination-preview-author' => __DIR__ . '/aae-a-post-pagination-preview-author.html.twig',
		];
	}
	...
	public function get_atomic_settings(): array {
		...
		if ( $post && ! empty( $post['author'] ) ) {
			$settings['text'] = $post['author'];
		} elseif ( class_exists( '\Elementor\Plugin' ) && \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
			$settings['text'] = __( 'Author Name', 'animation-addons-for-elementor' );
		} else {
			$settings['text'] = '';
		}
