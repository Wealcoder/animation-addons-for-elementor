...
/**
 * Image Advanced Animation panel — one Section containing one anchor
 * Text_Control. The JS-side <ResponsiveSection> dispatcher matches the
 * anchor's $$type and renders the full interactions repeater in place.
 */
final class Controls {
	...
	private function build_section(): Section {
		return Section::make()
			->set_label( Bootstrap::get_label( __( 'Image Advanced Animation', self::TD ) ) )
			->set_items( [
				Text_Control::bind_to( Schema::IMGADV_SECTION_ANCHOR ),
			] );
	}
}
