...
function parseBoxShadows(str) {
	...
	return parts.map(part => {
		...
		if (rest.toLowerCase().includes('inset')) {
			inset = true;
			rest = rest.replace(/inset/ig, '').trim();
		}
...
function stringifyBoxShadows(shadows) {
...
export function BoxShadow({ value, onChange, disabled }) {
	const [shadows, setShadows] = useState(() => parseBoxShadows(value));
	const [pickerAnchor, setPickerAnchor] = useState(null);
	const [activeShadowIdx, setActiveShadowIdx] = useState(null);

	useEffect(() => {
		setShadows(parseBoxShadows(value));
	}, [value]);

	const updateShadows = (newShadows) => {
		setShadows(newShadows);
		onChange(stringifyBoxShadows(newShadows));
	};
	...
	const addShadow = () => {
		updateShadows([...shadows, { x: 0, y: 0, blur: 0, spread: 0, color: '#000000', inset: false }]);
	};
			...
			{shadows.map((shadow, idx) => (
				<Box key={idx} sx={{ p: 1.5, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
					...
					<Stack direction="row" spacing={1} sx={{ mb: 1.5 }}>
						...
						<Box sx={{ flex: 1 }}>
							<Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>Spread</Typography>
							<TextField
								size="small" type="number" fullWidth disabled={disabled}
								value={shadow.spread} onChange={(e) => updateShadow(idx, 'spread', e.target.value)}
							/>
						</Box>
					</Stack>
					...
					<Stack direction="row" spacing={2} alignItems="center">
						...
						<Box>
							<Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>Inset</Typography>
							<Switch 
								size="small" 
